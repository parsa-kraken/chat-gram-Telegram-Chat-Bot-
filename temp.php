...
//error_reporting(0);
define('API_KEY', '5981414077:AAF26if8GA6biIIaFBsxqOvqKPZpeyuqJ1Y'); // توکن
define('ADS_TOKEN', '5887910453:AAGKqJS8ARCuRv0oq9QhxyjYdTdXrQejgl0');
//-----------------------------------------------------------------------------------------
...
//github : parsa-kraken
//web : www.parsakraken.ir
$admin = array("37204871", "734873978", "00000"); // ایدی ادمین ها را ماننده این الگورتیم بگذارید ادمین اصلی ایدی اول است
$usernamebot = "iRoChatbot"; // یوزرنیم ربات
$channel = "sourcepath"; // یوزرنیم کانال
$channelname = "sjsjs";
$web = "https://nextnumber.ir/testbot/"; // ادرس محل قرار گیری سورس
//-----------------------------------------------------------------------------------------------
// database 
// اطلاعات دیتا بیس را وارد کنید
$servername = "localhost";
$username = "nextnumb_test";
$password = 'Qf8t%MWle?hy';
$dbname = "nextnumb_test";
...
به ربات چت ناشناسه 《 آیرو چت بات 》 خوش آمدید 🌹
...
🌟 خوب اول باید اطلاعات خود را ثبت کنی ! جنسیت خودت رو انتخاب کن ؟ 👇🏻",
		...
		$connect->query("INSERT INTO user (id, step,step2, coin, member,  name , photo , sex , city , old , stats , `like` , dislike , blocklist , frindlist , gps , vip ,username, bio , baner , daily , side , liker , time) VALUES ('$from_id', 'setsex','none', '1000000', '0', '' , '' , '' , '' , '0' , '', '0', '0', '', '', '', 'true' ,'$username', '' , '' , '' , '0' , '' , '')");
	} else {
		if ($user["step"] != "chat") {
			jijibot('sendmessage', [
				'chat_id' => $chat_id,
...
} elseif ($user["step"] == "sendpmway1" && $tc == "private") {
	$getme = $user["step2"];
	jijibot('sendmessage', [
		'chat_id' => $chat_id,
...
🌟 برای مدیریت چت هم میتونی از دکمه های زیر استفاده کنی 👇🏻",
	...
	پیام شما به صورت ناشناس برای طرف مقابل ارسال میشود"
				...
				$connect->query("UPDATE user SET step ='sendpmnow' WHERE id = '$from_id' LIMIT 1");
	...
	پیام شما به صورت ناشناس برای طرف مقابل ارسال میشود"
				...
				$connect->query("INSERT INTO user (id, step,step2, coin, member,  name , photo , sex , city , old , stats , `like` , dislike , blocklist , frindlist , gps , vip ,username, bio , baner , daily , side , liker , time) VALUES ('$from_id', 'sendpmnow','$getuser', '1000000', '0', '$first_name' , '' , '' , '' , '0' , '', '0', '0', '', '', '', 'true' ,'$username', '' , '' , '' , '0' , '' , '')");
			}
		}
	}
} elseif ($user["step"] == "sendpmnow" && $tc == "private") {
	$getme = $user["step2"];
	jijibot('sendmessage', [
		'chat_id' => $chat_id,
		'text' => "✅ پیام شما با موفقیت به صورت ناشناس به طرف مقابل ارسال شد
...
💬 که وارد ربات 《 آیرو چت بات 》 بشی و به صورت #ناشناس با #دیگران , #اطرافیانت  و ... چت کنی !
💯 واقعی و رایگان
...
💎 لطفا بسته مورد نظر خود را انتخاب کنید تا به صفحه پرداخت منتقل شوید .",
		...
		jijibot('sendmessage', [
			"chat_id" => $chat_id,
			"text" => "$first_name عزیز 😄
💬 برای استفاده از ربات آیرو چت بات ابتدا باید وارد کانال 《 $channelname 》 شوید 👇🏻
...
☑️ بعد از عضویت بر روی دکمه عضو شدم ضربه بزنید تا ربات برای شما فعال شود",
	...
	if ($tch == 'member' or $tch == 'creator' or $tch == 'administrator') {
		...
		if ($count > 0) {
			...
			jijibot('sendmessage', [
				'chat_id' => $chat_id,
				'text' => "👨‍👧‍👧لیست دوستان شما در ربات آیرو چت بات :
...
☑️ بعد از عضویت بر روی دکمه عضو شدم ضربه بزنید تا ربات برای شما فعال شود",
	...
	if ($tch == 'member' or $tch == 'creator' or $tch == 'administrator') {
		...
		if ($count > 0) {
			...
			jijibot('sendmessage', [
				'chat_id' => $chat_id,
				'text' => "🚫 بلاک لیست شما در ربات آیرو چت بات
...
☑️ بعد از عضویت بر روی دکمه عضو شدم ضربه بزنید تا ربات برای شما فعال شود",
	...
	if ($tch == 'member' or $tch == 'creator' or $tch == 'administrator') {
		jijibot('sendmessage', [
			'chat_id' => $chat_id,
			'text' => "💡 راهنمای کامل ربات آیرو چت بات برای بخش های مختلف ربات .
...
",
		...
		jijibot('sendmessage', [
			"chat_id" => $chat_id,
			"text" => "$first_name عزیز 😄
💬 برای استفاده از رباتآیرو چت بات ابتدا باید وارد کانال 《 $channelname 》 شوید 👇🏻
...
ℹ️ میتوانید به استفاده از دکمه های زیر اقدام به تنظیم عکس پروفایل و بیو [درباره] کنید 👇🏻",
		...
		jijibot('sendmessage', [
			'chat_id' => $chat_id,
			'text' => "🎫 پروفایل شما در ربات آیرو چت بات :
...
🌟 به منوی اصلی ربات خوش اومدی . خوب الان از منوی پایین انتخاب کن 👇🏻",
	...
	$id = jijibot('sendphoto', [
		...
		'caption' => "$getnamesub دعوتت کرده
💬 که وارد ربات 《 آیرو چت بات 》 بشی و به صورت ناشناس با #اطرافیانت , #همسنات یا و ... چت کنی !
...
☑️ ثبت اطلاعات اولیه شما با موفقیت انجام شد به ربات 《 آیرو چت بات 》 خوش آمدید .
...
elseif ($update->message->text && $update->message->reply_to_message && $from_id == $admin[0] && $tc == "private") {
	...
	jijibot('sendmessage', [
		'chat_id' => $chat_id,
		'text' => "به ربات چت ناشناسه 《 آیرو چت بات 》 خوش آمدید 🌹
